...
var aim_y: float = 430.0
var aim_min_y: float = 345.0
var aim_max_y: float = 520.0
var aim_speed: float = 2.35

var penguins_left: int = 3
var launched_penguins: int = 0
var bounces_left: int = MAX_BOUNCES
var has_dived: bool = false
var projectile_pos: Vector2 = CATAPULT_POS
var projectile_vel: Vector2 = Vector2.ZERO
var projectile_spin: float = 0.0
var score: int = 0
...
var floating_messages: Array[Dictionary] = []

var ui_restart_rect: Rect2 = Rect2(520, 475, 240, 64)
var ui_next_rect: Rect2 = Rect2(520, 545, 240, 64)
...
func _process(delta: float) -> void:
	if state == GameState.AIMING:
		aim_time += delta
		var wave: float = (sin(aim_time * aim_speed) + 1.0) * 0.5
		aim_y = lerp(aim_min_y, aim_max_y, wave)
		projectile_pos = Vector2(CATAPULT_POS.x, aim_y)
	elif state == GameState.FLYING:
		var target_camera: float = clampf(projectile_pos.x - 390.0, 0.0, LEVEL_W - VIEW_W)
		camera_x = lerp(camera_x, target_camera, clampf(delta * 5.0, 0.0, 1.0))
...
func launch_penguin() -> void:
	...
	var height_factor: float = 1.0 - inverse_lerp(aim_min_y, aim_max_y, aim_y)
	var force_x: float = 780.0 + height_factor * 135.0
	var force_y: float = -650.0 - height_factor * 250.0
...
func _check_ground_collision() -> void:
	...
	var impact: float = projectile_vel.length()
...
func _check_seal_collisions() -> void:
	for seal in seals:
		if not seal.alive:
			continue
		var dist: float = projectile_pos.distance_to(seal.pos)
		var combined: float = PENGUIN_RADIUS + float(seal.radius)
		if dist <= combined:
			var impact: float = projectile_vel.length()
			var damage: float = impact * 0.16 + (70.0 if has_dived else 0.0)
			seal.hp -= damage
			score += int(damage)
			var normal: Vector2 = (projectile_pos - seal.pos).normalized()
...
func _check_block_collisions() -> void:
	for block in blocks:
		...
		var closest: Vector2 = Vector2(
			clampf(projectile_pos.x, rect.position.x, rect.position.x + rect.size.x),
			clampf(projectile_pos.y, rect.position.y, rect.position.y + rect.size.y)
		)
		if projectile_pos.distance_to(closest) <= PENGUIN_RADIUS:
			var impact: float = projectile_vel.length()
			var damage: float = impact * (0.10 + (0.05 if has_dived else 0.0))
			block.hp -= damage
			score += int(damage * 0.6)
			var normal: Vector2 = (projectile_pos - closest).normalized()
			if normal == Vector2.ZERO:
				normal = Vector2.UP
...
func _update_floating_messages(delta: float) -> void:
	...
	for msg: Dictionary in floating_messages:
		if float(msg.life) > 0.0:
			active_messages.append(msg)
	floating_messages = active_messages
...
func _draw_screen_background() -> void:
	for i in range(18):
		...
		var c: Color = Color(0.07 + t * 0.04, 0.04 + t * 0.10, 0.18 + t * 0.22)
...
func _draw_catapult() -> void:
	var wood: Color = Color(0.44, 0.24, 0.12)
	var dark_wood: Color = Color(0.25, 0.13, 0.08)
	var rope: Color = Color(0.95, 0.78, 0.45)
	var left_anchor: Vector2 = CATAPULT_POS + Vector2(-42, -85)
	var right_anchor: Vector2 = CATAPULT_POS + Vector2(44, -85)
...
func _draw_penguin(pos: Vector2, spin: float, diving: bool) -> void:
	var body: Color = Color(0.05, 0.07, 0.11)
	var belly: Color = Color(0.95, 0.98, 1.0)
	var beak: Color = Color(1.0, 0.66, 0.10)
	var accent: Color = Color(0.52, 0.32, 1.0)
	...
	draw_circle(pos + Vector2(11, -9), 2.0, Color.BLACK)
	var beak_pts: PackedVector2Array = PackedVector2Array([pos + Vector2(-9, 0), pos + Vector2(9, 0), pos + Vector2(0, 12)])
	draw_colored_polygon(beak_pts, beak)
	var wing_offset: float = 21.0 if not diving else 27.0
...
func _draw_seal(seal: Dictionary) -> void:
	...
	var hp_ratio: float = clampf(float(seal.hp) / float(seal.max_hp), 0.0, 1.0)
	var body: Color = Color(0.52, 0.58, 0.68)
	var belly: Color = Color(0.80, 0.87, 0.94)
	var dark: Color = Color(0.11, 0.12, 0.16)
	...
	draw_line(pos + Vector2(28, 18), pos + Vector2(48, 30), body, 9)
	# Barra de vida.
	var bar: Rect2 = Rect2(pos.x - 42, pos.y - float(seal.radius) - 22, 84, 8)
...
func _draw_ice_block(block: Dictionary) -> void:
	var rect: Rect2 = block.rect
	var hp_ratio: float = clampf(float(block.hp) / float(block.max_hp), 0.0, 1.0)
	var base: Color = Color(0.64, 0.88, 1.0)
	var damaged: Color = Color(0.45, 0.58, 0.72)
	var c: Color = damaged.lerp(base, hp_ratio)
	draw_rect(rect, c)
...
func _draw_float_texts() -> void:
	...
	for msg in floating_messages:
		var alpha: float = clampf(float(msg.life), 0.0, 1.0)
...
func _draw_ui() -> void:
	var font: Font = ThemeDB.fallback_font
	var panel: Color = Color(0.04, 0.03, 0.08, 0.55)
	var purple: Color = Color(0.48, 0.28, 1.0)
	var text: Color = Color(0.94, 0.91, 1.0)
...
func _draw_result_panel(font: Font, purple: Color, text: Color) -> void:
	var title: String = "FASE CONCLUÍDA!" if state == GameState.LEVEL_CLEAR else "TENTE NOVAMENTE"
	var subtitle: String = "Você derrubou todas as focas." if state == GameState.LEVEL_CLEAR else "Os pinguins acabaram antes de derrubar todas as focas."
	...
	var star_y: float = 337.0
	for i in range(3):
		var filled: bool = i < stars_earned
		var c: Color = Color(1.0, 0.78, 0.14) if filled else Color(0.28, 0.24, 0.34)
